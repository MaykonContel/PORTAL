...
def _init_tables(conn):
    with conn.cursor() as cur:
        cur.execute("""
            ...
            CREATE TABLE IF NOT EXISTS organizador_snapshots (
                id         SERIAL PRIMARY KEY,
            ...
            CREATE INDEX IF NOT EXISTS idx_organizador_snapshots_usuario_id
                ON organizador_snapshots (usuario, id DESC);
            -- Módulos genéricos (pipeline, os_romaneio, ordem_servico, cronograma)
            CREATE TABLE IF NOT EXISTS modulo_store (
                modulo     TEXT NOT NULL,
                key        TEXT NOT NULL,
                value      TEXT NOT NULL,
                updated_at TEXT NOT NULL,
                PRIMARY KEY (modulo, key)
            );
        """)
    conn.commit()
...
def _json_loads_safe(txt, default=None):
    try:
        return json.loads(txt) if isinstance(txt, str) and txt.strip() else default
    except Exception:
        return default

def _org_effective_count(data):
    """Conta conteúdo real do organizador para evitar sobrescrever dados com estado vazio."""
    if not isinstance(data, dict):
        return 0
    total = 0
    for key in ("habits", "dailies", "todos", "notes"):
        val = data.get(key)
        if isinstance(val, list):
            total += len(val)
    for key in ("events", "itemNotes", "itemFiles", "hlog", "dlog"):
        val = data.get(key)
        if isinstance(val, dict):
            total += len(val)
    kb = data.get("kanban")
    if isinstance(kb, dict):
        boards = kb.get("boards")
        if isinstance(boards, list):
            total += len(boards)
            for b in boards:
                if isinstance(b, dict):
                    cols = b.get("cols") or b.get("columns") or []
...
def _insert_org_snapshot(cur, usuario, data_text, motivo="auto"):
    if not data_text:
        return
    cur.execute(
        "INSERT INTO organizador_snapshots (usuario, data, motivo, created_at) VALUES (%s,%s,%s,%s)",
        (usuario, data_text, motivo, now_iso())
    )
    # Mantém os últimos 100 snapshots por usuário para não crescer infinito no plano grátis.
    cur.execute("""
        DELETE FROM organizador_snapshots
        WHERE usuario=%s AND id NOT IN (
            SELECT id FROM organizador_snapshots
            WHERE usuario=%s
            ORDER BY id DESC
            LIMIT 100
        )
...
def api_organizador_save():
    ...
    force_empty = bool(request.args.get("force") == "1" or (isinstance(body, dict) and body.get("__force_empty_save") is True))
    if isinstance(body, dict) and "__force_empty_save" in body:
        body = dict(body)
        body.pop("__force_empty_save", None)
    new_text = json.dumps(body, ensure_ascii=False)
    new_count = _org_effective_count(body)
    try:
        conn = get_db()
        with conn.cursor() as cur:
            cur.execute("SELECT data FROM organizador WHERE usuario=%s", (user,))
            old_row = cur.fetchone()
            old_text = old_row["data"] if old_row else None
            old_data = _json_loads_safe(old_text, {})
            old_count = _org_effective_count(old_data)

            # Proteção principal: se já existiam dados e o navegador tentou salvar estado vazio,
            # bloqueia a gravação para evitar perda total por falha de carregamento/sincronização.
            if old_count > 0 and new_count == 0 and not force_empty:
                _insert_org_snapshot(cur, user, old_text, "bloqueio_estado_vazio")
                conn.commit(); conn.close()
                return jsonify({
                    "ok": False,
                    "blocked": True,
                    "error": "Salvamento bloqueado: o organizador tentou gravar estado vazio sobre dados existentes.",
                    "old_count": old_count,
                    "new_count": new_count
                }), 409

            if old_text:
                _insert_org_snapshot(cur, user, old_text, "antes_de_salvar")
            cur.execute("""INSERT INTO organizador (usuario,data,updated_at) VALUES (%s,%s,%s)
                        ...
                        (user, new_text, now_iso()))
        conn.commit(); conn.close()
        return jsonify({"ok":True,"updated_at":now_iso(),"items":new_count})
    except Exception as e:
        return jsonify({"ok":False,"error":str(e)}),500

@api.get("/organizador/backups")
def api_organizador_backups():
    user = _current_user()
    if not user: return jsonify({"ok":False}),401
    try:
        conn = get_db()
        ...
        out = []
        for r in rows:
            d = _json_loads_safe(r["data"], {})
            out.append({"id": r["id"], "motivo": r["motivo"], "created_at": r["created_at"], "items": _org_effective_count(d)})
        return jsonify({"ok": True, "backups": out})
    except Exception as e:
        return jsonify({"ok":False,"error":str(e)}),500

@api.post("/organizador/restore")
def api_organizador_restore():
    user = _current_user()
    if not user: return jsonify({"ok":False}),401
    body = request.get_json(silent=True) or {}
    sid = body.get("snapshot_id")
    if not sid: return jsonify({"ok":False,"error":"snapshot_id obrigatório"}),400
    try:
        conn = get_db()
        with conn.cursor() as cur:
            cur.execute("SELECT data FROM organizador WHERE usuario=%s", (user,))
            ...
            if current and current.get("data"):
                _insert_org_snapshot(cur, user, current["data"], "antes_de_restaurar")
            cur.execute("SELECT data FROM organizador_snapshots WHERE usuario=%s AND id=%s", (user, sid))
            snap = cur.fetchone()
            if not snap:
                conn.close()
            ...
            cur.execute("""INSERT INTO organizador (usuario,data,updated_at) VALUES (%s,%s,%s)
                           ON CONFLICT (usuario) DO UPDATE SET data=EXCLUDED.data, updated_at=EXCLUDED.updated_at""",
                        (user, snap["data"], now_iso()))
        conn.commit(); conn.close()
        return jsonify({"ok":True,"data":_json_loads_safe(snap["data"], {})})
    except Exception as e:
        return jsonify({"ok":False,"error":str(e)}),500

# ── Pipeline (contel_v3) API ───────────────────────────────────────────────────
@api.get("/pipeline")
def api_pipeline_get():
    ...
    try:
        conn = get_db()
        with conn.cursor() as cur:
            cur.execute("SELECT key, value FROM modulo_store WHERE modulo = 'pipeline'")
            rows = cur.fetchall()
        conn.close()
        data = {}
        for row in rows:
            try: data[row["key"]] = json.loads(row["value"])
            except: data[row["key"]] = row["value"]
...
def api_pipeline_save():
    ...
    if not user: return jsonify({"ok":False}),401
    body = request.get_json(silent=True)
    if not isinstance(body, dict): return jsonify({"ok":False,"error":"JSON inválido"}),400
    try:
        conn = get_db()
        with conn.cursor() as cur:
            for key, value in body.items():
                cur.execute("""INSERT INTO modulo_store (modulo,key,value,updated_at) VALUES (%s,%s,%s,%s)
                               ON CONFLICT (modulo,key) DO UPDATE SET value=EXCLUDED.value, updated_at=EXCLUDED.updated_at""",
                            ("pipeline", key, json.dumps(value, ensure_ascii=False), now_iso()))
        conn.commit(); conn.close()
        return jsonify({"ok":True})
    except Exception as e:
        return jsonify({"ok":False,"error":str(e)}),500
...
# ── OS Romaneio API ────────────────────────────────────────────────────────────
@api.get("/os-romaneio")
def api_os_romaneio_get():
    user = _current_user()
    if not user: return jsonify({"ok":False}),401
    try:
        conn = get_db()
        with conn.cursor() as cur:
            cur.execute("SELECT key, value FROM modulo_store WHERE modulo = 'os_romaneio'")
            rows = cur.fetchall()
        conn.close()
        data = {}
        for row in rows:
...
@api.post("/os-romaneio")
def api_os_romaneio_save():
    user = _current_user()
    if not user: return jsonify({"ok":False}),401
    body = request.get_json(silent=True)
    if not isinstance(body, dict): return jsonify({"ok":False,"error":"JSON inválido"}),400
    try:
        ...
        with conn.cursor() as cur:
            for key, value in body.items():
                cur.execute("""INSERT INTO modulo_store (modulo,key,value,updated_at) VALUES (%s,%s,%s,%s)
                               ON CONFLICT (modulo,key) DO UPDATE SET value=EXCLUDED.value, updated_at=EXCLUDED.updated_at""",
                            ("os_romaneio", key, json.dumps(value, ensure_ascii=False), now_iso()))
        conn.commit(); conn.close()
        return jsonify({"ok":True})
    except Exception as e:
        return jsonify({"ok":False,"error":str(e)}),500
...
def api_ordem_servico_get():
    user = _current_user()
    if not user: return jsonify({"ok":False}),401
    try:
        conn = get_db()
        with conn.cursor() as cur:
            cur.execute("SELECT key, value FROM modulo_store WHERE modulo = 'ordem_servico'")
            rows = cur.fetchall()
        conn.close()
        data = {}
        for row in rows:
...
@api.post("/ordem-servico")
def api_ordem_servico_save():
    user = _current_user()
    if not user: return jsonify({"ok":False}),401
    body = request.get_json(silent=True)
    if not isinstance(body, dict): return jsonify({"ok":False,"error":"JSON inválido"}),400
    try:
        ...
        with conn.cursor() as cur:
            for key, value in body.items():
                cur.execute("""INSERT INTO modulo_store (modulo,key,value,updated_at) VALUES (%s,%s,%s,%s)
                               ON CONFLICT (modulo,key) DO UPDATE SET value=EXCLUDED.value, updated_at=EXCLUDED.updated_at""",
                            ("ordem_servico", key, json.dumps(value, ensure_ascii=False), now_iso()))
        conn.commit(); conn.close()
        return jsonify({"ok":True})
    except Exception as e:
        return jsonify({"ok":False,"error":str(e)}),500
...
# ── Cronograma API ─────────────────────────────────────────────────────────────
@api.get("/cronograma")
def api_cronograma_get():
    user = _current_user()
    if not user: return jsonify({"ok":False}),401
    try:
        conn = get_db()
        with conn.cursor() as cur:
            cur.execute("SELECT key, value FROM modulo_store WHERE modulo = 'cronograma'")
            rows = cur.fetchall()
        conn.close()
        data = {}
        for row in rows:
    ...
    try:
        ...
        with conn.cursor() as cur:
            for key, value in body.items():
                cur.execute("""INSERT INTO modulo_store (modulo,key,value,updated_at) VALUES (%s,%s,%s,%s)
                               ON CONFLICT (modulo,key) DO UPDATE SET value=EXCLUDED.value, updated_at=EXCLUDED.updated_at""",
                            ("cronograma", key, json.dumps(value, ensure_ascii=False), now_iso()))
        conn.commit(); conn.close()
        return jsonify({"ok":True})
    except Exception as e:
        return jsonify({"ok":False,"error":str(e)}),500
